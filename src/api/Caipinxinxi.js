...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let cailiao = req.query.cailiao
			if (cailiao) {

				if (cailiao.indexOf('%') != -1) {
					where.cailiao = {
						[Op.like]: cailiao
					}
				} else {
			...
			let xiangqing = req.query.xiangqing
			if (xiangqing) {

				if (xiangqing.indexOf('%') != -1) {
					where.xiangqing = {
						[Op.like]: xiangqing
					}
				} else {
			...
			let jieshao = req.query.jieshao
			if (jieshao) {

				if (jieshao.indexOf('%') != -1) {
					where.jieshao = {
						[Op.like]: jieshao
					}
				} else {
