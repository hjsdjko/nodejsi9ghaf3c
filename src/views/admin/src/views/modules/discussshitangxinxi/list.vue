		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 20px 0","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","margin":"0 0 0px","borderColor":"#f6f6f6","display":"flex","justifyContent":"center","minHeight":"100px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"#fff","borderWidth":"2px 2px 0","fontSize":"inherit","borderStyle":"solid"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">评论内容</label>
				...
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}'>
					<el-button :style='{"border":"2px solid #00acc120","cursor":"pointer","padding":"0 10px 0 4px","outline":"none","color":"#00acc1","borderRadius":"4px","background":"none","width":"auto","fontSize":"16px","lineHeight":"30px","height":"auto"}' type="success" @click="backClick()">返回</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('discussshitangxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discussshitangxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","borderColor":"#f6f6f6","margin":"0 0 0px","borderRadius":"0 0 4px 4px","background":"rgba(255,255,255,1)","borderWidth":"0 2px 2px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussshitangxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('discussshitangxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="btn8" type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-yanziwancheng" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","display":"none","height":"auto"}'></span>
								回复
							</el-button>
...
import AddOrUpdate from "./add-or-update";
	...
	this.$router.push({path:'/discussdiscussshitangxinxi',query:{refid:id}});
    },
	backClick(){
		history.back()
	},
	...
	discussshitangxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'discussshitangxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    async deleteHandler(id ,refid ) {
		...
		await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
			...
			await this.$http({
				url: "discussshitangxinxi/delete",
				method: "post",
				data: ids
			}).then(async ({ data }) => {
				if (data && data.code === 0) {
					this.$http({
						url: 'shitangxinxi/info/' + refid,
						method: "get",
					}).then(res=>{
						if(res.data&&res.data.code==0){
							res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
							this.$http({
								url: 'shitangxinxi/update',
								method: "post",
								data:res.data.data
							}).then(res1=>{
